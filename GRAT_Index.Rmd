...
```{r}
change_data <- change_data %>% mutate(RegionName = str_trim(str_extract(NAME, "(?<=,)[^,]+")))
```

``` {r}
## NOTE: There appears to be some lost rows in the combined_data_for_pca dataframe. Can you look into this?
combined_data_for_pca <- change_data %>% right_join(county_changes, by = "RegionName") %>% drop_na()
combined_data_for_pca <- combined_data_for_pca %>% drop_na()
```

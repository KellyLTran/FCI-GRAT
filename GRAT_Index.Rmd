...
append_year_suffix <- function(data, year) {
  ...
  data %>%
    rename_with(~ str_replace(.x, "$", suffix), -c(GEOID, NAME))
}
...
```{r} 
combined_data_for_pca <- right_join(tract_changes, change_data, by = "GEOID")
```
...
dependent_var <- combined_data_for_pca$over_under_2020_20  
